...
def check_amp(model):
    """
    Checks the PyTorch Automatic Mixed Precision (AMP) functionality of a YOLO11n model. If the checks fail, it means
    there are anomalies with AMP on the system that may cause NaN losses or zero-mAP results, so AMP will be disabled
    during training.

    Args:
        model (nn.Module): A YOLO11n model instance.
    ...
    Returns:
        (bool): Returns True if the AMP functionality works correctly with YOLO11n model, else False.
    """
