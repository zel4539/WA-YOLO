...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask,
        ...
        """IoU loss."""
        weight = target_scores.sum(-1)[fg_mask].unsqueeze(-1)
        iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=True, epoch=epoch, CIoU=True, UIoU=False,
                       Focal=False)
        # UIoU设置为True则代码使用UIoU, Focal同时设置为True 则设置的是FocalUIoU
        if type(iou) is tuple:
            if len(iou) == 2:
                # increased the weight of low/high IoU
                loss_iou = ((1 - iou[1].detach().squeeze()) * (
                            1 - iou[0].squeeze()) * weight).sum() / target_scores_sum  # Focal
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=10):  # model must be de-paralleled
        ...
        #                           self.hyp.Focaler, self.hyp.epochs, self.hyp.alpha).to(device)
